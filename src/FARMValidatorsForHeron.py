...
class FARM_SISO(Validator):
  ...
  def read_input(self, inputs):
    ...
    #TODO: print some error message here:
        ...
        if str(unit) not in str(comp):
          ...
          if self._unitInfo[unit]['StateVariableSelectionXML'] is None: # if the self learning was not done during SVS
            ...
            if len(self._unitInfo[unit]['t_hist']) == 0: # if self-learning was never run before
              t = -Tr_Update_sec*len(LearningSetpoints) # negative values means self-learning stage
          ...
          """ 5. Simulate the Dispatch stage using CG, and update the ABCD matrices as it goes (if demanded) """
          ...
          # MOAS steps Limit 
          g = 2*int(Tr_Update_sec/Tss)+1 # numbers of steps to look forward, , type = <class 'int'>
            ...
            for res in info:
              if str(res) == "electricity":
                ...
                for tidx, time in enumerate(times):
                  ...
                  v_adj = float(v_RG)+float(v_0) # absolute value of electrical power (MW)
                  print("\n**************************", "\n**** RG summary Start ****","\nUnit = ", str(unit),
                        ", t = ", t, "\nr = ", r_value, "\nProfile Selected = ", profile_id, "\nv_adj = ", v_adj, 
                        "\n***** RG summary End *****","\n**************************\n")
                  print("HaoyuRGSummary,", str(unit), ",t,", t, ",Profile,", profile_id, 
                    ",r0,", r_value, ",v0,", v_adj, ",v_min,", float(v_min)+float(v_0), ",v_max,", float(v_max)+float(v_0) )
                  ...
                  for i in range(int(Tr_Update_sec/Tss)):
                    ...
                    v_fetch = np.asarray(v_adj).reshape(m,)
                  ...
                  if comp.get_interaction().is_type('Storage'):
                    if tidx == 0: # for the first hour, use the initial level
                      Allowed_Level = Initial_Level - v_adj*Tr_Update_hrs # Allowed_Level: predicted level due to v_value
                    else: # for the other hours
                      Allowed_Level = Allowed_Level - v_adj*Tr_Update_hrs
                    V1 = Allowed_Level
                  else: # when not storage,
                    V1 = v_adj
